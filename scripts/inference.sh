      ...
      do
          if [ ! -d /data/prepared/$environment_id/$assignment_id/$date/${f: -12:-4}/*.json ]; then
              ...
              do
                  ...
                  do
                      ...
                      if [ $aquired -eq 1 ]; then
                          selected_gpu=$gpu
                          # expire the key in 5min, a video should never take that long
                          expired=$($REDIS EXPIRE $key 300)
                          break 2
                      fi
                  done
              ...
              if [[ ! "$selected_gpu" == "" ]]; then
                  ...
                  export GPU=$selected_gpu
                  RES=$(${CMD} --detector ${detector_type} --cfg ${alphapose_cfg_path} --checkpoint ${pose_model_weights} --sp --video ${f} --gpus ${selected_gpu} --outdir ${outdir} --format coco)
                  echo "inference completed [$RES]"
                  $REDIS del $key
                  set -e
              fi
          fi
