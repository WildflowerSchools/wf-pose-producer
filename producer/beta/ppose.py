...
def pose_nms(bboxes, bbox_scores, bbox_ids, pose_preds, pose_scores, areaThres=0):
    ...
    while mask.any():
        ...
        logging.info("deleted ids %s", delete_ids)
    ...
    for j, pick_item in enumerate(pick):
        ...
        merge_pose, merge_score = p_merge_fast(
            preds_pick[j], ori_pose_preds[merge_id], ori_pose_scores[merge_id], ref_dists[pick_item])
